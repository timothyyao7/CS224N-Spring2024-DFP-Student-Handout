...
# Evaluate multitask model on dev sets; include nli
def model_eval_multitask_nli(sentiment_dataloader,
                             paraphrase_dataloader,
                             sts_dataloader,
                             nli_dataloader,
                             model, device):
    ...
    with torch.no_grad():
        ...
        # Evaluate natural language inference.
        nli_y_true = []
        nli_y_pred = []
        nli_sent_ids = []
        ...
        print(f'Paraphrase detection accuracy: {paraphrase_accuracy:.3f}')
        print(f'Semantic Textual Similarity correlation: {sts_corr:.3f}')
        print(f'Natural Language Inference accuracy: {nli_accuracy:.3f}')

        return (sentiment_accuracy, sst_y_pred, sst_sent_ids,
                paraphrase_accuracy, para_y_pred, para_sent_ids,
                sts_corr, sts_y_pred, sts_sent_ids,
                nli_accuracy, nli_y_pred, nli_sent_ids)
