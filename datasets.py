...
def load_multitask_data_nli(sentiment_filename,paraphrase_filename,similarity_filename,inference_filename,split='train'):
    sentiment_data = []
    num_labels = {}
    if split == 'test':
        with open(sentiment_filename, 'r') as fp:
    ...
    inference_data = []
    if split == "test":
        with open(inference_filename, 'r', encoding='utf8') as fp:
            for record in csv.DictReader(fp,delimiter = ','):
                sent_id = record['id'].lower().strip()
                inference_data.append((preprocess_string(record['premise']),
        ...
        with open(inference_filename, 'r', encoding='utf8') as fp:
            for record in csv.DictReader(fp,delimiter = ','):
                ...
                inference_data.append((preprocess_string(record['premise']),
                                       preprocess_string(record['hypothesis']),
                                       int(float(record['label'])),sent_id))
                
    print(f"Loaded {len(inference_data)} {split} examples from {inference_filename}")

    return sentiment_data, num_labels, paraphrase_data, similarity_data, inference_data
